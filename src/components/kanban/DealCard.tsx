...
export function DealCard({ deal, isDragging, onClick, searchTerm = '' }: DealCardProps) {
  ...
  return (
    <div
      className={cn(
        "deal-card border border-border rounded-md p-2 mb-2 cursor-grab hover:shadow-lg hover:scale-102 transition-all duration-200",
        "w-52 min-h-[90px] select-none",
    ...
    >
      ...
      <div className="font-semibold text-sm text-foreground mb-1 leading-tight">
        {getHighlightedTextParts(truncate(deal.nome_fundo, 40), searchTerm).map((part, index) => (
          part.isHighlighted ? (
            <mark key={index} className="bg-yellow-200 dark:bg-yellow-800 px-0.5 rounded">{part.text}</mark>
          ) : (
      ...
      <div className="flex items-center gap-1 mb-1">
        <div className="text-green-600 font-medium text-sm">
          {formatCurrencyApproximate(deal.oferta_base || 0)}
        </div>
        <div className="text-muted-foreground/50">|</div>
        <div className="text-purple-600 font-medium text-sm">
          {formatCurrencyApproximate(deal.receita_potencial || 0)}
        </div>
      </div>

      {/* Line 3: 4 Colored Text Tags */}
      <div className="flex items-center gap-1 text-xs font-semibold truncate">
        <span className={vehicleColor.text || 'text-muted-foreground'}>
          {vehicleColor.label || 'N/A'}
        </span>
        <span className="text-muted-foreground/50">|</span>
        <span className={productColor.text || 'text-muted-foreground'}>
          {productColor.label || 'N/A'}
        </span>
        <span className="text-muted-foreground/50">|</span>
        <span className={sectorColor.text || 'text-muted-foreground'}>
          {sectorColor.label || 'N/A'}
        </span>
        <span className="text-muted-foreground/50">|</span>
        <span className={indexerColor.text || 'text-muted-foreground'}>
...
export function SortableDealCard({ deal, onClick, searchTerm = '' }: SortableDealCardProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
